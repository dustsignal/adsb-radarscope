...
4. **Set Data Source:** Change the `TAR1090_URL` to the URL of your `aircraft.json` file.
   * If ```adsb-radar.html``` is hosted on the same server as ```tar1090``` (or `dump1090-fa`), you can use a relative path like ```/tar1090/data/aircraft.json``` or ```/skyaware/data/aircraft.json```.
   * If you are accessing a ```tar1090``` or ```dump1090-fa``` instance on your local network, use its full URL (e.g., ```http://192.168.1.100/tar1090/data/aircraft.json``` or `http://192.168.1.100/skyaware/data/aircraft.json`), you may encounter CORS errors at this point depending on your specific setup.
> [!CAUTION]
